jQuery(document).ready(function(a) {
    ...
    a(document).bind("reset_image", function() {
        ...
        if (a("input.amount_needed, select.amount_needed").each(function(b, c) {
                ...
                if (h = e(h, d.data("unit"), d.data("common-unit")), "area-linear" === wc_price_calculator_params.measurement_type) c ? c += 2 * h : c = 2 * h;
                    ...
                    if (!c) {
                        ...
                        return k = e(f(k), a("#height_needed").data("unit"), a("#height_needed").data("common-unit")), void(c = 2 * (i * j + j * k + i * k))
                    }
                } else c ? c *= h : c = h
            ...
            var d = b(c);
            d ? (wc_price_calculator_params.product_price = parseFloat(d.price), a(".single_variation").html(d.price_html)) : (wc_price_calculator_params.product_price = "", a(".single_variation").html(""))
        }
        a("#_measurement_needed").val(c), a("#_measurement_needed_unit").val(wc_price_calculator_params.product_price_unit);
        var h = "";
        if (c ? (h = wc_price_calculator_params.product_price * c, wc_price_calculator_params.minimum_price > h && (h = parseFloat(wc_price_calculator_params.minimum_price)), a(".product_price").html(g(h)).trigger("wc-measurement-price-calculator-product-price-change", [c, h])) : a(".product_price").html("").trigger("wc-measurement-price-calculator-product-price-change"), a(".wc-measurement-price-calculator-total-amount")) {
            var i = e(c, wc_price_calculator_params.product_price_unit, a(".wc-measurement-price-calculator-total-amount").data("unit"));
            i = parseFloat(c.toFixed(wc_price_calculator_params.measurement_precision)), a(".wc-measurement-price-calculator-total-amount").text(i)
        }
        if ("undefined" != typeof woocommerce_addons_params && a("form.cart").find("#product-addons-total").length > 0) {
            var j = "" === h ? 0 : h;
            woocommerce_addons_params.product_price = j.toFixed(2), a("form.cart").trigger("woocommerce-product-addons-update")
        }
        ...
        var b = a(this).closest("form.cart");
        b.trigger("wc-measurement-price-calculator-update")
    }), a(document).bind("show_variation", function(b, c) {
    ...
    }), a("body").bind("updated_addons", function() {
        var b = a("form.cart"),
        ...
        wc_price_calculator_params.product_measurement_value && (a(".amount_needed, .amount_actual").each(function(b, d) {
            ...
            f = parseFloat((f * c).toFixed(2)), d.is("input") ? d.val(f) : d.text(f)
        }), a(".total_price").html(g(c * wc_price_calculator_params.product_price)).trigger("wc-measurement-price-calculator-quantity-total-price-change", [c, wc_price_calculator_params.product_price]))
        ...
        if (wc_price_calculator_params.product_measurement_value) {
            ...
            if (a("input.amount_needed").each(function(c, d) {
                ...
                d < parseFloat(wc_price_calculator_params.quantity_range_min_value) && (d = parseFloat(wc_price_calculator_params.quantity_range_min_value)), parseFloat(wc_price_calculator_params.quantity_range_max_value) && d > parseFloat(wc_price_calculator_params.quantity_range_max_value) && (d = parseFloat(wc_price_calculator_params.quantity_range_max_value)), a("input[name=quantity]").val(d), a(".amount_actual").each(function(b, c) {
                    ...
                    f = parseFloat((f * d).toFixed(2)), c.is("input") ? c.val(f) : c.text(f)
                }), a(".total_price").html(g(d * wc_price_calculator_params.product_price)).trigger("wc-measurement-price-calculator-total-price-change", [d, wc_price_calculator_params.product_price])
            }
        }
        ...
        c.trigger("wc-measurement-price-calculator-quantity-changed", [b.target.value])
    }), a("input[name=quantity]").bind("change", function(b) {
        var c = a(this).closest("form.cart");
        c.trigger("wc-measurement-price-calculator-quantity-changed", [b.target.value])
    }).change(), a(document).bind("show_variation", function(b, c) {
